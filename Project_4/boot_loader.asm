...
LoadnEx:
     ;读取内核
      ...
      mov ax,cs                ;段地址 ; 存放数据的内存基地址
      mov es,ax                ;设置段地址（不能直接mov es,段地址）
      mov bx, OffSetOfintPrg1  ;偏移地址; 存放数据的内存偏移地址
      mov ah,2                 ; 功能号
      mov al,4                 ;扇区数
      mov dl,0                 ;驱动器号 ; 软盘为0，硬盘和U盘为80H
      mov dh,0                 ;磁头号 ; 起始编号为0
      ...
      mov ax,0
      mov es,ax
      mov word [es:0d0h],100h
      mov word [es:0d2h],500h
      mov word [es:0d4h],300h
      mov word [es:0d6h],500h
      mov word [es:0d8h],500h
      mov word [es:0dah],500h
      mov word [es:0dch],700h
      mov word [es:0deh],500h
      mov ax,800h
      push ax
      mov ax,100h
      push ax
